...
class PokerHand
  ...
  def straight?
    ...
    (HAND_SIZE - 1).times do 
      checkup = checksum
      if cards_by_number[i + 1].worth == cards_by_number[i].worth + 1
        checkup += 1 
      end
...
hands = Hash.new(0)
1_000_000.times do
  hands[PokerHand.new(Deck.new).evaluate] += 1
end
hands.sort_by { |_, value| value }.each do |hand, freq|
  puts "#{hand}: #{freq}/1,000,000, #{freq.fdiv(10000)}%"
end
